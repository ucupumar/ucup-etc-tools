...
class YAddOutline(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for o in objs:
            if not hasattr(o, 'materials') or o.type in {'META'}: continue
            # Try to get outline modifiers
            m = [m for m in o.modifiers if m.type == 'SOLIDIFY' and m.material_offset > 0 and m.use_flip_normals]
            mat = None
            ...
            if mat:
                ...
                # Check active bsdf
                dif_bsdf = [n for n in mat.node_tree.nodes if n.bl_idname == 'ShaderNodeBsdfDiffuse']
                if dif_bsdf: dif_bsdf = dif_bsdf[0]
                emi_bsdf = [n for n in mat.node_tree.nodes if n.bl_idname == 'ShaderNodeEmission']
                if emi_bsdf: emi_bsdf = emi_bsdf[0]
                ...
                if any(outp.inputs[0].links):
                    ...
                    if l.from_node.bl_idname == 'ShaderNodeMixShader':
                        l.from_node.inputs[0].default_value = self.color[3]
